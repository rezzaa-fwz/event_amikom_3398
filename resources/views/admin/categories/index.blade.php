...
<body class="bg-slate-50 text-slate-900 flex min-h-screen">
    ...
    class="inline-block px-6 py-3 bg-indigo-600 text-white rounded-2xl font-bold shadow-lg shadow-indigo-100 hover:bg-indigo-700 active:scale-95 transition">
        ...
        <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
            <form action="{{ route('admin.categories') }}" method="GET" class="px-8 py-6 bg-slate-50/50 border-b flex gap-4">
    ...
    @csrf
    @method('DELETE')
    <button type="submit" class="p-2.5 bg-rose-50 text-rose-600 rounded-xl hover:bg-rose-600 hover:text-white transition" title="Delete">
        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
