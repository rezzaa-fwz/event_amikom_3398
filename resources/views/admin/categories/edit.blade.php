    ...
    <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm p-8">
        <form action="{{ route('admin.categories.update', $category->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-6">
                ...
                @if($category->image)
                <p class="text-sm text-slate-500 mt-2">Gambar saat ini: <a href="{{ (str_starts_with($category->image ?? '', 'http') ? $category->image : asset('storage/' . $category->image)) }}" target="_blank" class="text-indigo-600 hover:underline">Lihat</a></p>
                @endif
                @error('image')
                <p class="text-red-500 text-xs mt-2">{{ $message }}</p>
                @enderror
