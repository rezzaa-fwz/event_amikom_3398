...
<div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm p-8 max-w-3xl">
    <form method="POST" action="{{ route('admin.partners.update', $partner->id) }}" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="mb-4">
            ...
            <img src="{{ asset('storage/' . $partner->logo_path) }}" class="w-24 h-24 rounded-xl object-contain border border-slate-200 p-1">
        </div>
        @endif

        <!-- Logo -->
        <div>
            <label class="block text-sm font-bold text-slate-600 mb-2">{{ $partner->logo_path ? 'Ganti Logo' : 'Upload Logo' }}</label>
            ...
            <p class="text-xs text-slate-400 mt-1">Format: JPEG, PNG, JPG, GIF. Maksimal 2MB. Kosongkan jika tidak ingin mengubah.</p>
            @error('logo')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            ...
            <textarea name="description" rows="4"
                class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 outline-none transition @error('description') border-red-500 @enderror">{{ old('description', $partner->description) }}</textarea>
            @error('description')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
